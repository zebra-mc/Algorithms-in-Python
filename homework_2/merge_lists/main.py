...
Новые узлы для элементов результата не создаются: ответ собирается из узлов
исходных списков, меняются только ссылки next. При равных значениях берем узел
из list1 (сравнение <=), поэтому равные элементы идут в том порядке, в каком
стояли.
...
существующих узлов, новых узлов данных не создается; в первом способе создается
один служебный dummy. Используются несколько указателей (tail, head) и в первом
способе один фиктивный узел, их число от n и m не зависит. Сами списки занимают
O(n + m), но это память под входные данные.
